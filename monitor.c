...
monitor* init_monitor(int total_request){
    ...
    monitor_t->requests_count_arr = (unsigned int*)malloc(sizeof(unsigned int)*2);
    ...
    monitor_t->consumed_count_arr[T_X] = (unsigned int *)malloc(sizeof(unsigned int)* 2);
    //allocate memory for each consumer, rev_9
    monitor_t->consumed_count_arr[REV_9] = (unsigned int *)malloc(sizeof(unsigned int)* 2);
...
void *producer_general(void *args){
    ...
    while(true){
        ...
        if(sync_monitor->request_count == sync_monitor->max_requests-1){
            ...
            push_to_queue(sync_monitor->queue_size, GEN_REQ); // we will use 0 to denote a general request
            ...
            output_request_added(producerNames[GEN_REQ], producerAbbrevs[GEN_REQ], sync_monitor->requests_count_arr);
...
void *consumer_t_x(void *args){
    ...
    while(true){
        ...
        output_request_removed(consumerNames[T_X], producerNames[req_typ], 
        sync_monitor->consumed_count_arr[T_X][req_typ], sync_monitor->requests_count_arr[req_typ] 
        );
