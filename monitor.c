...
monitor* init_monitor(int total_request){
    ...
    monitor_t->barrier = (sem_t *)malloc(sizeof(sem_t));

    // initialize barrier semaphores
    sem_init(monitor_t->barrier,0, 0);// barrier sem for main thread barrier
...
void *producer_general(void *args){
    ...
    while(true){
        ...
        //acquire mutext lock/ wait for lock
        ...
        while(sync_monitor->request_count >= sync_monitor->max_requests|| sync_monitor->queue_size == MAX_QUEUE_SIZE)
        {
            //unlock
    //        printf("general is leaving\n");
            // if the production limit is met
            if(sync_monitor->request_count == sync_monitor->max_requests){
                // release the lock
                pthread_mutex_unlock(&sync_monitor->lock);
                sem_post(sync_monitor->barrier);
                return NULL;
            }
            // wait for available slots
            pthread_cond_wait(&sync_monitor->full, &sync_monitor->lock);// relenquish lock in case waiting
...
void* producer_vip(void * args){
    ...
    while(true){
        ...
        while(sync_monitor->request_count >= sync_monitor->max_requests || sync_monitor->queue_size == MAX_QUEUE_SIZE || 
         (sync_monitor->requests_count_arr[VIP_REQ] >= MAX_VIPS)){
            //unlock
        //    printf("VIP leaving\n");
        //    printf("max requests have been met, VIP leaving\n");
        //    fflush(stdout);
            // if production limit is met
            if(sync_monitor->request_count >= sync_monitor->max_requests){
                pthread_mutex_unlock(&sync_monitor->lock); // release lock
        ...
        while(sync_monitor->queue_size == 0){
          //  printf("The queue is empty, check if more requests are expected\n");
            ...
            //    fflush(stdout);
                ...
                pthread_mutex_unlock(&sync_monitor->lock);
                sem_post(sync_monitor->barrier); // signal barrier sem
                return NULL;
            }
